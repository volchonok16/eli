...
- [x] Главная страница с секциями: Hero, Benefits, FeaturedProducts, HowItWorks, Testimonials, FAQ
...
- [ ] Каталог товаров:
  ...
  - Сортировка: по популярности, цене (возр/убыв), новизне
...
- [ ] Карточка товара (`/products/:id`):
  ...
  - Блок «С этим покупают» (cross-sell, запрос `GET /api/products/:id/related`)
...
- [ ] :star: **Яндекс.Карты на фронтенде:**
  - Отображение всех активных точек (`isActive: true`)
  - Балун с информацией: название, адрес, режим работы
  - Фильтр по дате открытия/закрытия базара
  - :bulb: Яндекс.Карты API v3 — бесплатная квота 1000 запросов/день, достаточно для MVP
...
- [ ] :star: **Лендинг для арендодателей:**
  - Страница `/cooperation` — условия, выгоды, фото успешных точек
  - Форма подачи заявки: ФИО/организация, контакты, адрес участка, площадь, фото (multer)
...
- [ ] :star: **Таблица оптовых цен:**
  - Страница `/wholesale` — таблица: высота ёлки × объём закупки → цена
  - Данные из `WholesalePrice`
- [ ] :star: **Запрос коммерческого предложения (КП):**
  - Форма `POST /api/cp-requests` — контакты + реквизиты компании + загрузка файла (ИНН, ОГРН)
  - Уведомление менеджеру в Telegram
  - Генерация PDF с прайс-листом (post-MVP, на старте — ручная отправка менеджером)
...
- [ ] **Мобильная адаптация:**
  - :star: Mobile First — протестировать все страницы на iPhone SE и Samsung Galaxy
  - Touch-friendly элементы управления (min 44×44px)
  - Оптимизация изображений (srcset / WebP через MinIO image transformation или Sharp)
