...
export const ProductCard = ({ name, latinName, height, price, index, image }: ProductCardProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        <p className="text-xs text-text-muted/50 uppercase tracking-widest mb-2 font-medium">{latinName}</p>
        <h3 className="font-serif text-xl text-primary mb-2 group-hover:text-accent transition-colors duration-300 leading-tight">
          {name}
        </h3>
        <p className="text-text-muted text-sm mb-4">Высота: {height}</p>
        <p className="font-sans text-xl text-accent font-semibold">{price}</p>
      </div>
    </motion.article>
  );
};
